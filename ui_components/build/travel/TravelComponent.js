...
class TravelComponent extends React.Component {
  ...
  updateTravelDB(direction = 'none') {
    ...
    fetch("http://192.168.1.122/shinobi-chronicles2/shinobi-chronicles/api/travel_page/travel.php", //**TODO: Change this link */
    requestOptions).then(json => {
      return json.json();
    }).then(data => {
      /**Data Recieved do something */
      this.setState({
        playerID: data['response']['current_player_id']
      });
      console.log("Travel Component Errors: " + (data['errors'].length ? data['errors'] : 'No errors'));
    }).catch(e => {
      console.log("Travel Component API CALL Error: " + e);
    });
  } //Button Functions
  ...
  getTravelJSONData() {
    setInterval(() => {
      ...
      fetch("http://192.168.1.122/shinobi-chronicles2/shinobi-chronicles/api/travel_page/travel.php"
      /**TODO: Change this Link */
        ...
        this.setState({
          playerVillage: data['response']['village']
        });
        this.setState({
          userPosition: data['response']['position']
        });
        this.setState({
          userPosition_x: data['response']['pos_x']
        });
        this.setState({
          userPosition_y: data['response']['pos_y']
        });
        this.setState({
          village_position_data: data['response']['village_locations']
        });
      }).catch(e => {
        console.log("Travel Component Init API Call Error: " + e);
      });
    }, 600);
  }
  /**
  ...
  */
  ...
  getTileInfo(map_position, village_position_data) {
    //due to array logic
    var offset = 1;
    /*hardcoded villages according to the positions*/

    var village_names = ['Stone', 'Cloud', 'Leaf', 'Sand', 'Mist'];
    ...
    let tile_position = map_position.join("."); //if Village Position == current map position (return {village_data})
  ...
  renderMap(x, y, player_village_position, player_position_x, player_position_y, player_village_name) {
    ...
    for (var i = 0; i < x; i++) {
      ...
      for (var j = 0; j < y; j++) {
        data.push( /*#__PURE__*/React.createElement(MapSquare, {
          key: `${i}.${j}`,
  ...
  render() {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      colSpan: "18"
    }, "Your Location ", this.state.userPosition))), /*#__PURE__*/React.createElement("tbody", null, this.renderMap(mapSize_x, mapSize_y, this.state.village_position_data, this.state.userPosition_x, this.state.userPosition_y, this.state.playerVillage))), /*#__PURE__*/React.createElement("a", {
      style: travelButtonStyle,
      className: "east travelButton",
      onClick: this.moveEast
    }, /*#__PURE__*/React.createElement("div", {
      className: "rightArrow"
    })), /*#__PURE__*/React.createElement("a", {
      style: travelButtonStyle,
      className: "north travelButton",
      onClick: this.moveNorth
    }, /*#__PURE__*/React.createElement("div", {
      className: "upArrow"
    })), /*#__PURE__*/React.createElement("a", {
      style: travelButtonStyle,
      className: "south travelButton",
      onClick: this.moveSouth
    }, /*#__PURE__*/React.createElement("div", {
      className: "downArrow"
    })), /*#__PURE__*/React.createElement("a", {
      style: travelButtonStyle,
      className: "west travelButton",
